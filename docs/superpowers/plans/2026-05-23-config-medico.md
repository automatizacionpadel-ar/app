# Config Médico Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Crear la página `/dashboard/config` donde los médicos editan información personal, horarios y propiedades de citas.

**Architecture:** Server component carga el médico desde DB y lo pasa a un client component que maneja todo el estado. Un único PATCH endpoint actualiza el registro. Las fotos se suben directamente a Supabase Storage desde el cliente antes del submit.

**Tech Stack:** Next.js 14 App Router, Supabase DB + Storage, TypeScript, Tailwind CSS

---

### Task 1: Migración DB + bucket Storage

**Files:**
- Create: `supabase/migrations/<timestamp>_config_medico_fields.sql`

- [ ] Crear el archivo de migración:

```bash
supabase migration new config_medico_fields
```

Expected: `Created new migration at supabase/migrations/<timestamp>_config_medico_fields.sql`

- [ ] Escribir el SQL en el archivo recién creado (reemplazar su contenido completo):
...
- [ ] Aplicar la migración al remoto:
...
git commit -m "feat: add foto_perfil_url, logo_url, precio_consulta, acepta_agendamientos + storage bucket"
```
...
- [ ] Agregar los 4 campos nuevos a la interfaz `Medico` en `src/types/index.ts`, después de `webhook_token`:
...
La interfaz completa queda así:
...
git commit -m "feat: extend Medico type with config fields"
```

---

### Task 3: API endpoint PATCH /api/medicos/actualizar

**Files:**
- Create: `src/app/api/medicos/actualizar/route.ts`
...
```bash
git add src/app/api/medicos/actualizar/
git commit -m "feat: add PATCH /api/medicos/actualizar endpoint"
```
...
### Task 5: Client component ConfigCliente.tsx

**Files:**
- Create: `src/app/dashboard/config/ConfigCliente.tsx`
...
- [ ] Verificar que la ruta aparece en el build:

```bash
npm run build 2>&1 | grep -E "config|error|Error"
```

Expected: línea `ƒ /dashboard/config` sin líneas de error

- [ ] Commit:

```bash
git add src/app/dashboard/config/
git commit -m "feat: add /dashboard/config page with personal info, horarios and cita properties"
```
...
- [ ] Login como médico → navegar a `/dashboard/config`. Verificar: 3 secciones con datos precargados, sin errores en consola del browser.

- [ ] Subir foto de perfil: click en "Subir imagen" (avatar circular) → seleccionar JPG/PNG → verificar que el preview actualiza.

- [ ] Subir logo: click en "Subir imagen" (cuadrado) → seleccionar imagen → verificar preview.

- [ ] Editar nombre, celular, dirección. Activar/desactivar días en horarios. Cambiar horas. Cambiar precio de consulta. Togglear seña + monto. Togglear agendamientos. Click "Guardar cambios". Verificar que aparece mensaje verde "Cambios guardados correctamente".

- [ ] Recargar la página. Verificar que todos los cambios persisten.

- [ ] En Supabase Studio → Table Editor → medicos → verificar que el registro tiene los valores correctos en `foto_perfil_url`, `logo_url`, `precio_consulta`, `acepta_agendamientos`, `horarios`.

- [ ] Agregar `.superpowers/` al `.gitignore` si no está:
