...
async function generarPDF(
  ...
  // Logo del consultorio (si existe)
  let cursorX = margin
  const logoB64 = medico.logo_url ? await imgToBase64(medico.logo_url) : null
  if (logoB64) {
    try { doc.addImage(logoB64, 'JPEG', margin, 6, 26, 26) } catch {}
    cursorX = margin + 32
  }

  // Nombre médico y especialidad en header
  doc.setTextColor(255, 255, 255)
  doc.setFontSize(15)
  doc.setFont('helvetica', 'bold')
  doc.text(medico.nombre, cursorX, 17)
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(medico.especialidad, cursorX, 24)
  ...
  doc.text(nombrePaciente, margin + 4, y)
  if (paciente.celular) {
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(...gris)
    doc.text(paciente.celular, W - margin - 4, y, { align: 'right' })
  }
  y += 14
  ...
  // Sello
  const selB64 = medico.sello_url ? await imgToBase64(medico.sello_url) : null
  if (selB64) {
    try { doc.addImage(selB64, 'JPEG', margin, pieY + 4, 35, 22) } catch {}
  }

  // Firma
  const firB64 = medico.firma_url ? await imgToBase64(medico.firma_url) : null
  if (firB64) {
  ...
  doc.text(medico.nombre, W - margin - 28, pieY + 37, { align: 'center' })

  return doc.output('blob')
}

export default function RecetaModal({ paciente, medico, onClose }: Props) {
  ...
  const enviar = useCallback(async () => {
    ...
    try {
      ...
      // Generar PDF
      const blob = await generarPDF(medico, paciente, meds)

      // Subir a Storage
      const recetaId = crypto.randomUUID()
      const path = `${medico.id}/${recetaId}.pdf`
      ...
      const res = await fetch('/api/recetas/crear', {
        ...
        body: JSON.stringify({ paciente_id: paciente.id, medicamentos: meds, pdf_url: publicUrl }),
      })
        ...
        <div className="flex items-center justify-between px-5 py-4"
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: '#5C5C59' }}>{nombrePaciente}</p>
            </div>
          </div>
        ...
        {estado === 'ok' ? (
          <div className="flex flex-col items-center gap-3 py-10 px-5">
            ...
            <p className="text-xs text-center" style={{ color: '#5C5C59' }}>
              {nombrePaciente} recibirá una notificación push con el PDF de la receta.
            </p>
          ...
          <div className="p-5 flex flex-col gap-4">
            ...
            {!paciente.push_activo && (
              <p className="text-xs rounded-lg px-3 py-2"
                style={{ background: 'rgba(245,158,11,0.1)', color: '#F59E0B', border: '1px solid rgba(245,158,11,0.2)' }}>
                Este paciente no tiene notificaciones push activas. Podrás enviar la receta pero no recibirá una notificación.
              </p>
            )}
