...
export default function NuevoNegocioPage() {
  ...
  const [form, setForm] = useState({
    ...
    prompt_personalidad: 'Sos el asistente virtual del negocio. Sos amable, profesional y conciso.',
    tono: 'amigable', idioma: 'es',
  ...
  async function handleSubmit() {
    ...
    setError(null)
    try {
      const res = await fetch('/api/negocios/crear', {
        method: 'POST',
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold" style={{ color: '#F0F0EE' }}>Nuevo negocio</h1>
      ...
      <div className="rounded-xl p-6 mb-6"
        ...
        {paso === 1 && (
          <>
            <SeccionHeader icono={<Briefcase size={18} />} titulo="Datos del negocio" subtitulo="Información pública del negocio" />
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Campo label="Nombre *">
                  <Input value={form.nombre} onChange={e => setField('nombre', e.target.value)} placeholder="Mi Negocio" />
                </Campo>
                <Campo label="Rubro *">
                  <Input value={form.rubro} onChange={e => setField('rubro', e.target.value)} placeholder="Servicios, Salud, etc." />
                </Campo>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <Campo label="Email">
                  <Input type="email" value={form.email} onChange={e => setField('email', e.target.value)} placeholder="contacto@negocio.com" />
                </Campo>
              </div>
              ...
              <Campo label="Descripción">
                <Textarea value={form.descripcion} onChange={e => setField('descripcion', e.target.value)} placeholder="Breve descripción del negocio..." rows={3} />
              </Campo>
              <div className="grid grid-cols-2 gap-4">
                <Campo label="Duración de turno (min)">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm" style={{ color: '#F0F0EE' }}>
                  Requiere seña para confirmar el turno
                </span>
              </div>
              {form.requiere_sena && (
                <>
                  ...
                  <Campo label="Titular de la cuenta">
                    <Input value={form.titular_cuenta} onChange={e => setField('titular_cuenta', e.target.value)} placeholder="Nombre del titular" />
                  </Campo>
                </>
              )}
            </div>
        ...
        {paso === 4 && (
          <>
            ...
            <div className="space-y-4">
              <Campo label="Personalidad del agente" hint="Describí cómo debe comportarse el asistente con los clientes">
        ...
        {paso === 6 && (
          <>
            <SeccionHeader icono={<Briefcase size={18} />} titulo="Credenciales de acceso" subtitulo="El negocio usará estas credenciales para entrar al dashboard" />
            <div className="space-y-4">
              <Campo label="Email de acceso *">
                <Input type="email" value={form.email_acceso} onChange={e => setField('email_acceso', e.target.value)} placeholder="admin@negocio.com" />
              </Campo>
              ...
              <div className="rounded-lg p-4"
                ...
                <p className="text-xs" style={{ color: '#7AB619' }}>
                  Se creará un usuario en Supabase Auth con estas credenciales. El negocio podrá cambiar su contraseña desde el dashboard.
                </p>
              </div>
            </div>
          </>
        ...
        {paso < 6 ? (
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            {loading ? <span className="animate-pulse-soft">Creando...</span> : (
              <><CheckCircle size={15} /> Crear negocio</>
            )}
          </button>
        )}
      </div>
