...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await admin.from('conversacion_etiquetas').select('etiqueta_id, etiquetas(id,nombre,color)').eq('conversacion_id', params.id)
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!et) return NextResponse.json({ error: 'Etiqueta no válida' }, { status: 403 })
  const { error } = await admin.from('conversacion_etiquetas').insert({ conversacion_id: params.id, etiqueta_id }).select()
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const admin = createAdminClient()
  await admin.from('conversacion_etiquetas').delete().eq('conversacion_id', params.id).eq('etiqueta_id', etiquetaId)
  return NextResponse.json({ ok: true })
}
