...
export async function POST(req: NextRequest) {
  ...
  const { cliente_id, puntos, motivo } = await req.json()
  const admin = createAdminClient()
  const { error } = await admin.from('fidelizacion_puntos').insert({ negocio_id: negocioId, cliente_id, puntos: Number(puntos), motivo })
